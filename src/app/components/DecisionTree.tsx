...
export function DecisionTree({ choices }: DecisionTreeProps) {
  ...
  return (
    <section className="rounded-3xl bg-white/80 p-5 shadow-md backdrop-blur sm:p-8">
      <div className="mb-8 text-center">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-[#a7715b] sm:text-sm">
          Decision tree
        </p>
        <h3 className="font-['Oleo_Script',cursive] text-4xl text-[#754635] sm:text-5xl">
          The path you took
        </h3>
      </div>
      ...
      <div className="mx-auto max-w-2xl">
        {scenes.map((scene, idx) => {
          ...
          return (
            <div key={scene.scene}>
              <div className="flex justify-center">
                <div className="rounded-2xl bg-[#a7715b] px-5 py-3 text-center shadow-md">
                  <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#f3e8dc]/80 sm:text-xs">
                    Scene {scene.scene}
                  </p>
                  <p className="text-sm font-bold text-[#f3e8dc] sm:text-base">
                    {scene.title}
                  </p>
                </div>
              </div>
              ...
              <div className="flex justify-center py-2" aria-hidden="true">
                <ChevronDown className="h-5 w-5 text-[#bd8f79]" />
              </div>
        ...
        <div className="my-6 flex justify-center" aria-hidden="true">
          <ChevronDown className="h-5 w-5 text-[#a7715b]" />
        </div>

        <div className="mb-4 flex justify-center">
          <span className="rounded-full bg-[#dd9385]/30 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-[#754635]">
            Outcomes
          </span>
        </div>
    ...
    <div className="relative">
      <button
        ...
        className={`w-full rounded-2xl border-2 p-3 text-left transition-all sm:p-4 ${
          isChosen
            ? `${colors.border} ${colors.bg} shadow-sm`
            : 'border-[#d6b9a0]/60 bg-[#d6b9a0]/15 opacity-60 hover:opacity-100 focus:opacity-100'
        }`}
      >
        <div className="mb-2 flex items-center gap-2">
          <span
            className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${
              isChosen ? colors.dot : 'bg-[#bd8f79]'
            }`}
          >
            {letter}
          </span>
          {isChosen && <CheckCircle2 className={`h-4 w-4 ${colors.check}`} />}
        </div>
        <p
          className={`text-sm font-bold leading-snug ${
            isChosen ? colors.text : 'text-[#754635]/55'
          }`}
        >
          {option.label}
        </p>
      ...
      {isActive && (
        <div className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 w-44 -translate-x-1/2 rounded-xl bg-[#754635] px-3 py-2.5 text-xs leading-relaxed text-[#f3e8dc] shadow-xl sm:w-56">
          <div
            className="absolute -top-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-[#754635]"
            aria-hidden="true"
          />
          {option.blurb}
        </div>
  ...
  return (
    <div className="relative">
      <button
        ...
        style={
          isUser
            ? { backgroundColor: outcome.bg, color: outcome.text }
            : undefined
        }
        className={`w-full rounded-2xl p-4 text-center transition-all ${
          isUser
            ? 'shadow-lg'
            : 'border-2 border-[#d6b9a0]/60 bg-[#d6b9a0]/15 text-[#754635]/55 opacity-60 hover:opacity-100 focus:opacity-100'
        }`}
      >
        {isUser && (
          <div className="mb-2 flex justify-center">
            <CheckCircle2 className="h-5 w-5" style={{ color: outcome.text }} />
          </div>
        )}
      ...
      {isActive && (
        <div className="pointer-events-none absolute left-1/2 top-full z-20 mt-2 w-56 -translate-x-1/2 rounded-xl bg-[#754635] px-3 py-2.5 text-xs leading-relaxed text-[#f3e8dc] shadow-xl">
          <div
            className="absolute -top-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-[#754635]"
            aria-hidden="true"
          />
